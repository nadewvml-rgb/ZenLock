...
public class LockScreenActivity extends AppCompatActivity {
    ...
    protected void onPause() {
        ...
        // Reliability fix: The overlay (OverlayLockService) is the primary enforcement
        // layer during a lock session. AppBlockerService returns the user to this activity
        // on every window change. We do NOT race-restart from here, which was the root
        // cause of the "blinking" bug (two systems fighting over the foreground).
        //
        // We only need to handle two legitimate exit paths:
        //   1. A whitelisted app was explicitly launched from this screen.
        //   2. The screen turned off (handled naturally — overlay stays, no wake loop).

        if (isLaunchingWhitelistedApp) {
            isLaunchingWhitelistedApp = false;
            Log.d("LockScreenActivity", "Whitelisted app launch — pausing without restart.");
            return;
        }

        // Keep overlay visible whenever we are paused during a live lock session.
        // This is instant and covers the gap before AppBlockerService fires.
    ...
    protected void onStop() {
        super.onStop();
        // Overlay handles visual lock enforcement. AppBlockerService will re-launch
        // this activity when the user tries to open any non-whitelisted app.
        // No action needed here — attempting to restart from onStop was the source
        // of the home-button flicker loop.
        Log.d("LockScreenActivity", "onStop — overlay remains active via OverlayLockService");
    }
    ...
    protected void onDestroy() {
        ...
        // Cancel target countdown handler
    ...
    // ---------------------------------------------------------------------------
    // Target countdown — reads targets.json from res/raw, finds the next
    // upcoming target, and ticks a small "Xd Xh Xm Xs" counter every second.
    // ---------------------------------------------------------------------------
    private void setupTargetCountdown() {
        ...
        // Find next upcoming target (first one whose datetime is in the future)
        ...
        for (int i = 0; i < targets.length(); i++) {
            try {
                ...
                String datetimeStr = t.getString("datetime");
                ...
                Log.w("LockScreen", "Skipping malformed target entry", e);
            }
        }

        if (nextTarget == null) {
            // All targets are in the past — hide the card
            card.setVisibility(android.view.View.GONE);
            return;
        }

        // Show card with label
        try {
            labelView.setText(nextTarget.getString("label"));
        } catch (Exception e) {
            labelView.setText("Target");
        }
        card.setVisibility(android.view.View.VISIBLE);

        // Tick every second using a Handler — very low battery impact
        ...
        targetRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (diff <= 0) {
                    ...
                    // Don't reschedule — session is here
                    return;
                }
    ...
    private JSONArray loadTargetsJson() {
        try {
            // Load from res/raw/targets.json
    ...
    private void finishLockScreen() {
        // Remove the overlay view when the session ends cleanly
